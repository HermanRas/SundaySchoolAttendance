...
if (isset($_POST['classdate'])) {
    ...
    $sql = "SELECT id,name,surname FROM student WHERE class_id = $classid ORDER BY name;";
    ...
    <div class="form-style-5">
        <form method="POST">
            ...
            <fieldset>
                ...
                $classid = $_SESSION['class_id'];
                include_once('db_open.php');
                $sql = "SELECT id,name,surname FROM student WHERE class_id = $classid and active != 0 ORDER BY name;";
                $result = $conn->query($sql);
                foreach ($result as $row) {
                    //set options
